...
public class HttpInferenceClient implements InferenceClient {
    ...
    public HttpInferenceClient(AppProperties properties) {
        ...
        client = RestClient.builder().baseUrl(properties.ai().baseUrl().replaceAll("/+$", "") + "/").requestFactory(factory).build();
    }
    @Override
    public InferenceResult predict(Path imagePath) {
        MultipartBodyBuilder body = new MultipartBodyBuilder();
        MediaType mediaType = imagePath.toString().toLowerCase().endsWith(".png") ? MediaType.IMAGE_PNG : MediaType.IMAGE_JPEG;
        body.part("image", new FileSystemResource(imagePath)).contentType(mediaType);
